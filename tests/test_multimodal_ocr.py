...
class TestMultimodalOCRResult:
    ...
    def test_multimodal_result_to_dict(self):
        ...
        assert data["processing"]["pipeline"] == [
            "tesseract",
            "vision_analysis",
            "llm_correction",
        ]
...
class TestMultimodalOCR:
    ...
    def test_process_image_ocr_only(
        self, mock_preprocess, mock_data, mock_string, mock_open, mock_validate
    ):
    ...
    def test_process_image_with_correction(
        ...
        # Mock correction
        with patch.object(
            ocr.corrector, "correct", return_value=("修正されたテキスト", 2)
        ):
            with tempfile.NamedTemporaryFile(suffix=".png", delete=False) as tmp:
                tmp_path = tmp.name
    ...
    @patch("jtext.core.multimodal_ocr.requests.post")
    @patch("jtext.core.multimodal_ocr.ImagePreprocessor.preprocess")
    def test_process_image_full_multimodal(
        self,
        mock_preprocess,
        mock_post,
        mock_data,
    ...
    def test_multimodal_fusion_fallback(self, mock_post):
        ...
        vision_analysis = {"analysis": "技術文書の画像です。", "model": "llava"}
        ...
        try:
            ...
            with pytest.raises(Exception):
                ocr._multimodal_fusion_correction(
                    "テストテキスト", vision_analysis, tmp_path
                )
    ...
    def test_clean_multimodal_response(self):
        ...
        short_response = "短い"
        cleaned = ocr._clean_multimodal_response(
            short_response, "これは長い元のテキストです"
        )
    ...
    def test_context_aware_fusion_fallback(self):
        ...
        with patch.object(
            ocr.context_corrector,
            "correct_with_context",
            return_value=("修正されたテキスト", 5),
        ):
            result = ocr._context_aware_fusion_fallback("OCRテキスト", vision_analysis)

            corrected_text, corrections_applied, correction_ratio, fusion_method = (
                result
            )
